...
async function deploy(deployOptions, effects = defaultEffects) {
  ...
  effects.clack.intro(`${inverse(" observable deploy ")} ${faint(`v${"1.13.2"}`)}`);
...
class Deployer {
  ...
  async getUpdatedDeployConfig() {
    ...
    if (deployConfig.projectSlug && !deployConfig.projectSlug.match(/^[a-z0-9-]+$/)) {
      throw new CliError(
        `Found invalid \`projectSlug\` in ${join(this.deployOptions.config.root, ".observablehq", "deploy.json")}: ${deployConfig.projectSlug}.`
      );
    }
